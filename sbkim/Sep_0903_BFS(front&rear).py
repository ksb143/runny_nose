# BFS 너비 우선 탐색
# 자신과 가까운 노드부터 탐색하는 방법으로 그렇기 때문에 큐를 활용하여 많이 탐색한다.
# 7 8 1
# 1 2 1 3 2 4 2 5 4 6 5 6 6 7 3 7


# 반복문 활용3-1 (front, rear) - 인접행렬

def bfs(S, V):
    # 방문할 정점의 목록을 저장하는 queue를 만들어주기
    queue = [None] * (V+1)
    front = rear = -1
    rear += 1
    queue[rear] = S
    # 방문 여부 표시하기 위한 visited 만들기
    visited = [0] * (V+1)
    # front와 rear가 만나면 큐에 있는 연결된 정점 다 방문했다는 것
    while rear != front:
        # 정점에서 방문해서 길 찾고 길 찾으면 좀 있다가 방문할거니까 방문목록에 추가
        # 나머지 길 계속 찾기
        front += 1
        node =  queue[front]
        visited[node] = 1
        print(node, end=' ')
        # 인접 행렬에서 연결된 정점 찾기
        for i in range(1, V+1):
            if adj[node][i] and not visited[i]:
                # 큐에 추가
                rear += 1
                queue[rear] = i
                visited[i] = 1
    print()


# V는 노드 개수, E는 연결된 정점 개수, S 시작할 노드
V, E, S = map(int, input().split())
# 연결된 정점
edges = list(map(int, input().split()))
# 인접행렬
adj = [[0] * (V+1) for _ in range(V+1)]
for i in range(0, E*2, 2):
    adj[edges[i]][edges[i+1]] = 1
    adj[edges[i+1]][edges[i]] = 1

bfs(S, V)


# 반복문 활용3-2 (front, rear) - 인접리스트

def bfs2(S, V):
    # 방문할 정점의 목록을 저장하는 queue를 만들어주기
    queue = [None] * (V+1)
    front = rear = -1
    rear += 1
    queue[rear] = S
    # 방문 여부 표시하기 위한 visited 만들기
    visited = [0] * (V+1)
    # front와 rear가 만나면 큐에 있는 연결된 정점 다 방문했다는 것
    while rear != front:
        # 정점에서 방문해서 길 찾고 길 찾으면 좀 있다가 방문할거니까 방문목록에 추가
        # 나머지 길 계속 찾기
        front += 1
        node =  queue[front]
        visited[node] = 1
        print(node, end=' ')
        # 인접 리스트에서 연결된 정점 찾기
        for i in adj[node]:
            # i번 노드에 방문하지 않았으면
            if not visited[i]:
                # 큐에 추가
                rear += 1
                queue[rear] = i
                # 방문 여부 표시
                visited[i] = 1
    print()


# V는 노드 개수, E는 연결된 정점 개수, S 시작할 노드
V, E, S = map(int, input().split())
# 연결된 정점
edges = list(map(int, input().split()))
# 인접 리스트
adj = [[] for _ in range(V+1)]
for i in range(0, E*2, 2):
    v1, v2 = edges[i*2], edges[i*2+1]
    adj[v1].apend(v2)
    adj[v1].apend(v1)

bfs2(S, V)


# 반복문 활용3-3 (front, rear) - 원형큐이자 인접 행렬
# 원형큐는 front == rear인 경우 empty
# 원형큐는 rear 다음 칸이 front면 full
# 즉, 늘 front 자리는 비어있음

def bfs3(S, V):
    # 방문할 정점의 목록을 저장하는 queue를 만들어주기
    queue = [None] * 10
    front = rear = 0
    rear += 1
    queue[rear] = S
    # 방문 여부 표시하기 위한 visited 만들기
    visited = [0] * (V+1)
    # front와 rear가 만나면 큐에 있는 연결된 정점 다 방문했다는 것
    while rear != front:
        # 정점에서 방문해서 길 찾고 길 찾으면 좀 있다가 방문할거니까 방문목록에 추가
        # 나머지 길 계속 찾기
        front += (front + 1) % len(queue)
        node =  queue[front]
        visited[node] = 1
        print(node, end=' ')
        # 인접 리스트에서 연결된 정점 찾기
        for i in range(V+1):
            # i번 노드에 방문하지 않았으면
            if adj[node][i] and not visited[i]:
                # 큐에 추가
                rear += (rear + 1) % len(queue)
                queue[rear] = i
                # 방문 여부 표시
                visited[i] = 1
    print()


# V는 노드 개수, E는 연결된 정점 개수, S 시작할 노드
V, E, S = map(int, input().split())
# 연결된 정점
edges = list(map(int, input().split()))
# 인접행렬
adj = [[0] * (V+1) for _ in range(V+1)]
for i in range(0, E*2, 2):
    adj[edges[i]][edges[i+1]] = 1
    adj[edges[i+1]][edges[i]] = 1

bfs3(S, V)


# 반복문 활용3-4 (front, rear) - 원형큐이자 인접 리스트
# 원형큐는 front == rear인 경우 empty
# 원형큐는 rear 다음 칸이 front면 full
# 즉, 늘 front 자리는 비어있음

def bfs4(S, V):
    # 방문할 정점의 목록을 저장하는 queue를 만들어주기
    queue = [None] * 10
    front = rear = 0
    rear += 1
    queue[rear] = S
    # 방문 여부 표시하기 위한 visited 만들기
    visited = [0] * (V+1)
    # front와 rear가 만나면 큐에 있는 연결된 정점 다 방문했다는 것
    while rear != front:
        # 정점에서 방문해서 길 찾고 길 찾으면 좀 있다가 방문할거니까 방문목록에 추가
        # 나머지 길 계속 찾기
        front += (front + 1) % len(queue)
        node =  queue[front]
        visited[node] = 1
        print(node, end=' ')
        # 인접 리스트에서 연결된 정점 찾기
        for i in adj[node]:
            # i번 노드에 방문하지 않았으면
            if not visited[i]:
                # 큐에 추가
                rear += (rear + 1) % len(queue)
                queue[rear] = i
                # 방문 여부 표시
                visited[i] = 1
    print()


# V는 노드 개수, E는 연결된 정점 개수, S 시작할 노드
V, E, S = map(int, input().split())
# 연결된 정점
edges = list(map(int, input().split()))
# 인접 리스트
adj = [[] for _ in range(V+1)]
for i in range(0, E*2, 2):
    v1, v2 = edges[i*2], edges[i*2+1]
    adj[v1].apend(v2)
    adj[v1].apend(v1)

bfs4(S, V)